# Game Server Framework Design

> 去中心化、Go/C++ 双语言、Protobuf3 协议的游戏服务器框架设计文档。
> 版本：v0.2
> 状态：草案（持续迭代中）
...
- **设计要点**：
  ...
  - **多游戏适配**：心跳间隔、广播策略、压缩阈值、AOI 过滤规则均可通过配置热切换
...
- **设计要点**：
  ...
  - 统一抽象 `IRealtimeContext` 接口，不同容器类型实现不同子类，通过 `game_mode` 配置动态加载
  - 支持回放、观战、断线重连缓存
...
| **L2 游戏数值表** | 道具、关卡、掉落、技能数值 | `item_table`、`level_exp` | 中，运营活动时更新 |
...
3. **演进平滑**：早期可用单 Registry + 单 Etcd 跑通 MVP；生产期 Registry 多实例负载均衡，Etcd 3 节点集群即可满足多数游戏服规模。
...
| **新增实时模式** | Realtime Server（C++）内按游戏类型实现 Room/Scene 插件，动态加载（SO/DLL） |
| **分区分服/跨服** | Registry 增加 `region` `zone` 标签，路由层按标签过滤 |
| **热更新** | Go 服可用 plugin 机制；C++ 服可用动态库加载；Gateway/Realtime 推荐滚动更新 |

### 6.1 多游戏类型适配设计

框架从底层设计上需同时支撑 **MOBA（高频竞技）**、**SLG（策略大地图）**、**MMORPG（大世界社交）** 以及后续的卡牌、回合制、FPS 等类型。核心思路是：**协议统一、服务复用、实时层插件化、数据层按需扩展**。
...
- **房间/场景分配**：Registry 的 `metadata` 中标记 `game_mode = "sync_room"`，Gateway/Biz 按此标签路由
...
Gateway 通过读取 `game_mode` 配置动态选择上述参数，所有策略封装为 `IGatewayPolicy` 接口。
