...
function Start-Registry {
    Start-ServiceProcess "Registry" (Join-Path $binDir "registry-go.exe") "-listen 127.0.0.1:2379 -store memory"
}
function Start-DBProxy {
    Start-ServiceProcess "DBProxy" (Join-Path $binDir "dbproxy-go.exe") "-listen 127.0.0.1:3307 -redis 127.0.0.1:6379 -mysql root:123456@tcp(127.0.0.1:3306)/gmaker"
}
function Start-Biz {
    Start-ServiceProcess "Biz" (Join-Path $binDir "biz-go.exe") "-listen 127.0.0.1:8082 -registry 127.0.0.1:2379 -metrics :9082"
}
function Start-Gateway {
    Start-ServiceProcess "Gateway" (Join-Path $binDir "gateway-cpp.exe") "8081 127.0.0.1:2379 127.0.0.1:8082"
}
function Start-LogStats {
    Start-ServiceProcess "LogStats" (Join-Path $binDir "logstats-go.exe") "-listen 127.0.0.1:8085 -http :8086"
}
...
Write-Host ""
Write-Host "Done. Use .\scripts\stop-services.ps1 to stop." -ForegroundColor Green
